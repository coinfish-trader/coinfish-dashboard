...
def calc_squeeze(ticker: str) -> dict:
    ...
    try:
        ...
        # Drop the in-progress / unsettled trailing bar(s) yfinance returns with
        # NaN OHLC, otherwise .iloc[-1] lands on an empty bar and the scan
        # reports no squeeze/momentum data for every ticker.
        if df is not None:
